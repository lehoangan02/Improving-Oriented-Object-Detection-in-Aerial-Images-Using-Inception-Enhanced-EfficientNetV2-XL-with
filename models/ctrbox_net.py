    ...
    def forward(self, x):
        ...
        for head in self.heads:
            ...
            if 'hm' in head or 'cls' in head:
                dec_dict[head] = torch.sigmoid(dec_dict[head])
        return dec_dict
class CTRBOX_ExplicitCornersV2(nn.Module):
    def __init__(self, heads, pretrained, down_ratio, final_kernel, head_conv):
        super().__init__()
        channels = [3, 64, 256, 512, 1024, 2048]
        assert down_ratio in [2, 4, 8, 16]
    ...
    def forward(self, x):
        ...
        for head in self.heads:
            ...
            if 'hm' in head or 'cls' in head:
                dec_dict[head] = torch.sigmoid(dec_dict[head])
        return dec_dict
class CTRBOX_CombinedV1(nn.Module):
    def __init__(self, heads, pretrained, down_ratio, final_kernel, head_conv):
        super().__init__()
        channels = [3, 64, 256, 512, 1024, 2048]
        assert down_ratio in [2, 4, 8, 16]
        ...
        self.heads = heads
        self.aux_heads = heads
        self.adapt_aux = nn.Sequential(nn.Conv2d(640, 256, kernel_size=1, stride=1, padding=0, bias=True),
                                        nn.BatchNorm2d(256),
                                        nn.ReLU(inplace=True))
    ...
    def forward(self, x):
        ...
        for head in self.aux_heads:
            ...
            if 'hm' in head or 'cls' in head:
                aux_dict[head] = torch.sigmoid(aux_dict[head])
        return dec_dict, aux_dict
class CTRBOX_CombinedV2(nn.Module):
    def __init__(self, heads, pretrained, down_ratio, final_kernel, head_conv):
        super().__init__()
        channels = [3, 64, 256, 512, 1024, 2048]
        assert down_ratio in [2, 4, 8, 16]
        ...
        self.heads = heads
        self.aux_heads = heads
        self.adapt_aux = nn.Sequential(nn.Conv2d(256, 256, kernel_size=1, stride=1, padding=0, bias=True),
                                        nn.BatchNorm2d(256),
                                        nn.ReLU(inplace=True))
    ...
    def forward(self, x):
        ...
        upscaled = F.interpolate(x[-2], size=(152, 152), mode='bilinear', align_corners=False)
        upscaled = self.adapt_aux(upscaled)
